...
- Row R16's token-exposure scan cannot read a prefetch response the router issues but the interceptor could not buffer (an unbuffered prefetch). That one class is excluded from the row's residue check on purpose; it is the honest limit of spec § 7.2 row 4, not a gap the row hides.
