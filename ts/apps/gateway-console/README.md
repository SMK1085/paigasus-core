...
- **Browser** (`tests/e2e`, Playwright). `gateway-console-ts:test-e2e` runs three Playwright projects, selected by file name:
  ...
  - **`playground`** (SMA-635) — the REAL `paigasus-gateway` binary, built from the commit under test by `paigasus-gateway-rs:e2e-bin` (never cached), configured only through `GATEWAY_*` variables, with its own fake IAM and a mock OpenAI server that the test controls step by step. No container. It covers rows R22 through R28: chunk-by-chunk streaming with the gateway's own `auth=oidc` log line, Stop, the `insufficient-permissions` deny, the self-query wiring, the 401 without a session, a failure inside a record, and the token-leak scan of the chat route, and row R30 for the model-access grant of SMA-676. The fake IAM is not Cedar: the rows prove the wiring, not the decision.

  Each row is one test. `tests/unit/e2e-rows.test.ts` holds the full row list (R1–R7, plus R4b and R7's 403 control, then R8–R12, then R13–R21 for the settings pages of SMA-636, then R22–R29 for the playground of SMA-635 (R29, the streaming-off composer, is in the single-zone project), then R30 for the model-access grant of SMA-676, in the playground project). It fails when a row is missing, duplicated, or renamed.
...
- Acceptance criterion 3 is delivered by SMA-635: the organization page links to `/gateway/orgs/<org>/playground`, which streams a chat completion through `POST /gateway/api/chat` with the user's own bearer. A person needs the `gateway_user` role on the organization. An organization admin grants and revokes it for a person in the "Model access for people" section of the organization page (SMA-676). The playground shows "You need the gateway_user role on this organization" until then. `org_admin` alone does not hold `InvokeModel`.
- A grant or a revoke of `gateway_user` is not immediate on every IAM replica (SMA-676 spec §9). A replica with the memory cache can keep the old decision for about 31 s. A person just revoked can still call models for up to about 31 s, and a person just granted access can still see "You need the gateway_user role on this organization" in the playground for the same span.
...
- There is no "Stop model calls" control: `RevokeRole` needs a grant id, and the service-account panel does not read the account's grants. Archive the account or revoke its keys instead.
