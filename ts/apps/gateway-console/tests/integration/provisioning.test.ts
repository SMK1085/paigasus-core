...
// app's resolver makes one WhoAmI call (SMA-632) carrying the request correlation id, and that
// myScopes() through this app's real runtime picks memberships-only vs. memberships+grants off the
...
describe("this app's resolver (lib/auth.ts)", () => {
  it('makes one WhoAmI call, carrying the request correlation id', async () => {
    ...
    const calls = env.iam.calls.slice(before).filter((call) => call.method === 'authn.whoAmI');

    expect(calls).toHaveLength(1);
    expect(calls[0]?.correlationId).toBe(correlationId);
  });
});
...
describe("myScopes() through this app's runtime", () => {
  it('returns memberships only when IAM reports no iam.authz.cedar', async () => {
    ...
    env.iam.setHandlers({
      ...
      'authn.whoAmI': () => ({ memberships: [{ id: 'm1', principalPrn: 'prn:pgs:iam:::principal/p1', nodePrn: organizationPrn(IDS.orgA) }] }),
