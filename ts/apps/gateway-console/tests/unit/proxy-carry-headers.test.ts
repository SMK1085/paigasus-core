...
describe('proxy carries the auth middleware allow response', () => {
  const res = proxy(new NextRequest('https://console.example.test/gateway/overview', { nextConfig: { basePath: '/gateway' } }));
  ...
  it('still forwards the two request headers proxy.ts mints', () => {
    ...
    expect(res.headers.get('x-middleware-request-x-paigasus-request-path')).toBe('/gateway/overview');
  });
});
