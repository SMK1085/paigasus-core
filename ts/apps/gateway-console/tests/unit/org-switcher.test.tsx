  ...
  return renderToStaticMarkup(
    <ZoneProvider zone="gateway" zones={{ gateway: '/gateway' }}>
      <SessionProvider value={{ principalPrn: null, displayName: 'Ada', email: null, grants: [], grantsAvailable: false }}>
        <OrgSwitcherShell brand={{ label: 'Paigasus AI Gateway', href: '/gateway/overview' }} nav={[]} orgs={ORGS}>
          <p>page</p>
        </OrgSwitcherShell>
      </SessionProvider>
    </ZoneProvider>,
...
describe('the organization switcher', () => {
  it('builds full-path hrefs under the basePath', () => {
    expect(orgSwitcherItems(ORGS, '/gateway')).toEqual([
      { id: ACME, label: 'Acme', href: `/gateway/orgs/${ACME}` },
      { id: GLOBEX, label: 'Globex', href: `/gateway/orgs/${GLOBEX}` },
    ]);
  });
