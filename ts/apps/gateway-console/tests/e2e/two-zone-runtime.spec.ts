...
// nowhere else.
//
// Do NOT assert a raw count of WhoAmI calls for one navigation. After hydration, the Next
// router prefetches every visible link, and each prefetch is another request and therefore
// another legitimate render and another legitimate WhoAmI call. A raw count is flaky in the
...
// IAM on every call, so the fake's call log carries it. Group the WhoAmI calls by that id.
...
// the SAME request makes a second memoization identity, so its two WhoAmI calls carry the
// SAME correlation id and land in the SAME group, which is exactly what the last assertion below
// catches and what a raw count would only catch by luck. `currentPrincipal()` calls WhoAmI, not
// Introspect (SMA-632), so this row now groups WhoAmI calls instead.
//
...
test('R11: no single request makes more than one WhoAmI call (§ 5.3)', async ({ page, harness }) => {
  ...
  const calls = harness.iam.callsTo('authn.whoAmI');
  ...
  // The invariant: ONE runtime per request means ONE WhoAmI per request, however many requests ran.
