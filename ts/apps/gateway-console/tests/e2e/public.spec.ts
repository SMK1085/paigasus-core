...
test('R1: the public page /gateway/ loads with no cookie, with its CSS and JS (shell)', async ({ page, harness }) => {
  const assets: { pathname: string; status: number; type: string }[] = [];
  page.on('response', (response) => {
    const url = new URL(response.url());
    if (url.pathname.startsWith('/gateway/_next/static/')) assets.push({ pathname: url.pathname, status: response.status(), type: response.request().resourceType() });
  });

  const response = await page.goto(harness.url('/gateway/'));

  expect(response?.status()).toBe(200);
  // Next may normalise /gateway/ to /gateway (trailingSlash is off). Either way the proxy did not redirect to login.
  expect(new URL(page.url()).pathname).toMatch(/^\/gateway\/?$/);
