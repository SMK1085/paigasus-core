...
restores `next-env.d.ts` and removes the `AGENTS.md`/`CLAUDE.md` files Next generates, so it leaves
...
- No page and no Server Action skips its IAM call because `mayI()` said no. `mayI()` (`@paigasus/console-core`'s `authorize.ts`) only hides a button, a form, a section or a nav entry. IAM decides.
...
- **Unit and integration** (`tests/unit`, `tests/integration`, vitest). The integration tests talk real gRPC to an in-process fake IAM (`@paigasus/console-core`'s `testing/fake-iam.ts`), and MSW serves `GET /v1/service-info`. Loaders (`load.ts`) take their IAM client and `mayI` as arguments, and commands (`commands.ts`) take their IAM client only, so the tests call them with no session and no Next runtime.
...
- The 403 view shows the correlation id only when `FORBIDDEN_VIEW_CORRELATION` (`@paigasus/console-core`'s `src/correlation.ts`) is `'header'`. When it is `'fallback'`, `callIam` logs the id with the path, and the section and form 403s show it.
- The console layout calls `myScopes()` on every console page for the organization switcher. That is one `WhoAmI` (SMA-632), up to ten `ListRoleGrants` pages and up to 50 tenancy reads per render. The cost is not measured (spec § 12). If it is too slow, a short-lived per-session cache is the next step.
- `@paigasus/console-core`'s `src/prn-tenancy.ts` is NO LONGER an ADR-0005 exception (SMA-634). The PRN grammar lives once, in `paigasus-kernel`, and this module calls it through `@paigasus/kernel`; what stays here is IAM's tenancy rule, a UUID guard for a URL segment and a length limit for the FFI boundary. `tests/unit/prn-tenancy.test.ts` replays every vector of the kernel parity corpus, and `tests/unit/prn-tenancy-delegation.test.ts` mocks the kernel and fails if the file reads a PRN itself. Both files moved out of this app's `lib/` into the shared package (SMA-512 PR 2).
