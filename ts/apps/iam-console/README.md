...
- `@paigasus/console-core`'s `src/prn-tenancy.ts` is a recorded ADR-0005 exception (decision D6, fallback C). Its `tests/unit/prn-tenancy.test.ts` holds it to the kernel: it replays every vector of the kernel parity corpus. Both files moved out of this app's `lib/` into the shared package (SMA-512 PR 2).
