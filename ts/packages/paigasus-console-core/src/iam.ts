...
export const currentSession: () => Promise<ResolvedSession> = cache(async () => requireSession(await consolePorts().authRuntime()));
...
export const optionalSession: () => Promise<ResolvedSession | null> = cache(async () => getSession(await consolePorts().authRuntime()));
