...
- The `pull_request` filter of `images.yml` lists the image build inputs. For `rs/` these are
  ...
  `ts/pnpm-workspace.yaml`, `ts/package.json`, `ts/.npmrc` and `ts/apps/*/lib/config.ts`. It also
  lists `ci/images/**`, the workflow, `.prototools` and the two `.proto/plugins/*.toml` files. A PR
  that changes one of these runs the workflow automatically.
