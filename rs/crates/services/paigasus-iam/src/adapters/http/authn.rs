...
mod tests {
    ...
    ///
    /// Driven off `all_authn_errors()`, so a new `AuthnError` variant is covered automatically.
    /// The extractor's own codes (`RejectionKind` in `adapters::http::json`, SMA-587) are no
    /// longer this test's concern — they moved out with the extractor and are covered by
    /// `json::tests::every_request_extractor_code_is_in_the_registry` instead.
    #[tokio::test]
