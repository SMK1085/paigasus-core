    ...
    C: Clock,
{
    ...
    pub async fn create(&self, org: Uuid, slug: &str, name: &str, actor: &PrincipalId) -> Result<NodeView<Team>, TenancyError> {
        ...
        let entry = self.team_entry(Action::CreateTeam, &view, &stamp, corr, detail);
...
mod tests {
    use super::*;
    use crate::application::fakes::{BumpSnapshotBumper, CountingGenBumper, FakeAuditLog, FakeOutbox, FakeUnitOfWork, FixedClock, InMemoryOrgs, InMemoryTeams, SeqIds, TenancyStore, test_stamp};
    ...
    async fn each_team_mutation_emits_one_event_and_one_entry() {
        ...
        assert_eq!(create_events[0].event_type, EventType::TeamCreated);
        ...
        assert_eq!(create_entries[0].action, Action::CreateTeam.as_wire());
        assert_eq!(create_entries[0].detail["slug"], "eng");
        assert_eq!(create_entries[0].detail["name"], "Engineering");
    ...
    async fn the_emitted_actions_match_the_action_vocabulary() {
        ...
        // restore but never the EVENT TYPE — `EventType::TeamRestored` was asserted nowhere in
        // the repository before this. Pin it here too.
        let event_types: Vec<EventType> = outbox.0.lock().unwrap().iter().map(|e| e.event_type).collect();
        assert_eq!(event_types, vec![EventType::TeamCreated, EventType::TeamRenamed, EventType::TeamArchived, EventType::TeamRestored,]);
    }
    ...
    async fn a_no_op_archive_emits_nothing_but_a_real_restore_emits() {
        ...
        svc.restore(id, &actor).await.unwrap();
        let events = outbox.0.lock().unwrap().clone();
        assert_eq!(events.len(), 1, "restoring an archived team is a real transition");
        assert_eq!(events[0].event_type, EventType::TeamRestored);
        let entries = audit.0.lock().unwrap().clone();
    ...
    /// `NodeArchived` on rename — same variant `rename` uses for an own-archived node
    /// (`ports.rs:161`; `organizations.rs`'s own archived-rename tests answer `NodeArchived`
    /// too, `:561`/`:684`). `ParentArchived` is `create`'s variant, not `rename`'s. The name
    /// check runs before the transaction opens, so it must win.
    #[tokio::test]
    async fn a_bad_name_outranks_an_ancestor_archived_node() {
        ...
        // The guard is real: a VALID name on this team is refused as NodeArchived.
        assert_eq!(svc.rename(id, None, Some("Platform"), &actor(2)).await.unwrap_err(), TenancyError::NodeArchived);
