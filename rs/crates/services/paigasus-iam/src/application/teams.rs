    ...
    C: Clock,
{
    ...
    pub async fn create(&self, org: Uuid, slug: &str, name: &str) -> Result<NodeView<Team>, TenancyError> {
        ...
        let team = Team::new(id, slug, name, now)?;
...
mod tests {
    use super::*;
    use crate::application::fakes::{FixedClock, InMemoryOrgs, InMemoryTeams, SeqIds, TenancyStore};
    ...
    fn seed_org(store: &TenancyStore, uuid: u128, slug: &str, now: DateTime<Utc>) -> Uuid {
        let id = Uuid::from_u128(uuid);
        let org = Organization::new(OrganizationId::from_uuid(id), Slug::parse(slug).unwrap(), "Org", now).unwrap();
    ...
    async fn create_team_under_missing_or_archived_org_fails() {
        ...
        assert_eq!(svc.create(Uuid::from_u128(1), "eng", "Engineering").await.unwrap_err(), TenancyError::NotFound);

        // Effectively-archived org -> ParentArchived (fake honors the port's in-txn guard).
        let org = seed_org(&store, 9000, "acme", Utc::now());
        InMemoryOrgs(store.clone()).set_status(org, NodeStatus::Archived, Utc::now()).await.unwrap();
        assert_eq!(svc.create(org, "eng", "Engineering").await.unwrap_err(), TenancyError::ParentArchived);
    }

    #[tokio::test]
    async fn team_effective_status_follows_org() {
        let store = TenancyStore::default();
        let org = seed_org(&store, 9001, "acme", Utc::now());
        let svc = new_service(store.clone());

        let created = svc.create(org, "eng", "Engineering").await.unwrap();
        ...
        let archived = svc.archive(team_id).await.unwrap();
        ...
        InMemoryOrgs(store.clone()).set_status(org, NodeStatus::Active, Utc::now()).await.unwrap();
    ...
    async fn duplicate_slug_is_conflict_scoped_to_org() {
        let store = TenancyStore::default();
        let org1 = seed_org(&store, 9002, "acme", Utc::now());
        let org2 = seed_org(&store, 9003, "beta", Utc::now());
        let svc = new_service(store.clone());

        svc.create(org1, "eng", "Engineering").await.unwrap();
        assert_eq!(svc.create(org1, "eng", "Eng 2").await.unwrap_err(), TenancyError::SlugConflict);
        // The same slug under a different org is fine — uniqueness is scoped per org.
        svc.create(org2, "eng", "Engineering").await.unwrap();
    }

    #[tokio::test]
    async fn archive_is_idempotent_and_restore_reverses() {
        let store = TenancyStore::default();
        let org = seed_org(&store, 9004, "acme", Utc::now());
    ...
    async fn rename_rejects_empty_change_and_effectively_archived_team() {
        let store = TenancyStore::default();
        let org = seed_org(&store, 9005, "acme", Utc::now());
        let svc = new_service(store.clone());
    ...
    async fn lists_are_ordered_and_paginated() {
        let store = TenancyStore::default();
        let org = seed_org(&store, 9006, "acme", Utc::now());
        let clock = FixedClock::default();
        let t0 = Utc.timestamp_opt(1_700_000_000, 0).unwrap();
        clock.set(t0);
        let svc = TeamService::new(InMemoryTeams(store.clone()), SeqIds::default(), clock.clone());

        let a = svc.create(org, "alpha", "Alpha").await.unwrap();
        clock.set(t0 + Duration::seconds(1));
        let b = svc.create(org, "bravo", "Bravo").await.unwrap();
        clock.set(t0 + Duration::seconds(2));
        let c = svc.create(org, "charlie", "Charlie").await.unwrap();
