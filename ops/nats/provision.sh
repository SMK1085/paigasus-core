...
# `local -n` (nameref) below needs bash >= 4.3; macOS ships 3.2 as `/bin/bash`. Guard first so an
# operator running this locally gets an actionable message instead of a raw syntax error.
