...
- The login ends on the IAM console, but IAM shows as unusable: IAM refused the token. Read `logs/paigasus-*-iam-backend-*.log` for the JWKS fetch or the `aud` check (spec F3, F4). A wrong or missing `aud` is the `info` line "its aud claim holds none of the accepted audiences" (at most one line per 10 seconds).
