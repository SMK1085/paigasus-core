...
REQUIRED_REPO_TASKS = (
    ...
    "release-parity-py",
    "release-parity-ts",
    # SMA-593. Same reasoning as the three release-parity* entries above: this gate carries a
...
SELF_TASK_EXPECTED_GLOBS = {
    ...
    # SMA-593. Two globs, no literal files — MEASURED against moon 2.5.3, which reports this
    # task with an inputGlobs object holding both entries (plus the injected workspace-config
    # glob, filtered above) and NO inputFiles key at all. The first glob is deliberately
    # identical to workflow_credentials.py's SCAN_GLOB, which moon.yml's own comment on this
    # task records: scheduling and scanning must not be able to drift apart, so narrowing or
    # renaming it silently reopens the bypass-by-file-rename this gate exists to close. The
    # second is what makes WORKFLOW_CREDENTIALS_SH_CALL_SITES below reachable — drop it and
...
SELF_SCHEDULED_GATES = {
    ...
    # SMA-593 — repo:workflow-credentials. FOUR lines, like version-lockstep and unlike the
    # three-line gates: a `--self-test` invocation as well as a `--negative-control`, because the
    # rule table lives in the checker (in-process) while the wiring rows live in run.sh, and
    # neither half proves the other can report red. All three modes go through run.sh rather than
    # calling the checker directly, so each gets the uv preflight — `python3
    # workflow_credentials.py` would fail on `import yaml` — and the wrapper's 3 -> 1 exit-code
    # translation. Whole-line matched like every other entry, and here that is load-bearing in the
    # usual direction: `bash ci/workflow-credentials/run.sh` is a strict PREFIX of BOTH flagged
    # lines, so under a substring test deleting the REAL RUN would leave this pin green while CI
    # only ever ran a fixture table and a synthetic control and never read the repo's workflows.
    # `set -euo pipefail` is exactly as load-bearing as any invocation: Moon takes a `script:`
    # block's status from its LAST command, so without it a failing self-test or control is masked
    # by the passing real run.
    "workflow-credentials": (
        "set -euo pipefail",
...
# SMA-512, fix wave (final review). The tailwind-source guard checks ONE app per invocation
# (`--app <dir>`), so an app that never invokes it is simply unguarded — the silent skip the
# parameterization was meant to remove, moved rather than closed. Every ts/apps/* directory (one
# with a `package.json` — the same test ci/next-env/run.sh applies) must therefore be a member of
# this set, and its Moon project's RESOLVED `test` script must carry all three lines the guard
# needs: the self-test and the negative control prove the assertion can fire, and the real run for
# THAT app is the assertion.
#
# This is a SET of app names now, not a dict of hand-copied lines. The original shape stored three
# literal lines per app, so an entry for a second app could read `--app ts/apps/iam-console` —
# copied from the only existing entry — and MEASURE AS PASSING, because nothing tied the stored
# text to the key it lived under. The three expected lines are now DERIVED from the app name by
...
def _expected_tailwind_lines(app):
    """The three ci/tailwind-source/run.mjs invocation lines `app`'s `test` script must carry.

    DERIVED, not stored, so a registry entry can no longer name another app's `--app` directory
    (fix-wave finding A) and an entry can no longer register with fewer than three lines (the old
    per-app tuple could hold any length; a set membership check cannot).
    """
    run = "node ../../../ci/tailwind-source/run.mjs"
    return (
        f"{run} --self-test",
        f"{run} --negative-control",
        f"{run} --app ts/apps/{app}",
    )
...
ACTIONLINT_SH_INDENTED_CALL_SITES = (
    ...
    # SMA-603 fix wave (C2) — `release_plan_sh`'s one-line BODY. Every entry above pins a CALL to
    # that wrapper; the wrapper itself was pinned by nothing, so replacing its body with `true`
    # (or `:`) disabled ALL of check 11 — the arity floor, both self-test invocations and the
    # --assert run — while every pin here stayed byte-identical and the gate exited 0. Pinned
...
# SMA-593 — the same class again, for repo:workflow-credentials' --negative-control block, and
# pinned for the same measured reason RELEASE_PARITY_SH_CALL_SITES is: SELF_SCHEDULED_GATES pins
...
# ACTIONLINT_SH_CALL_SITES documents. The `printf` line above it carries the distinctive text, and
# every pinned line was verified to occur EXACTLY ONCE in run.sh before this tuple was written.
#
# REACHABILITY IS NOT AUTOMATIC, same as release-parity: this check only runs when
# repo:affected-smoke is scheduled, so moon.yml lists `ci/workflow-credentials/**/*` among its
# inputs and ci/actionlint/run.sh's T_AFFECTED_SMOKE_REQUIRED_INPUTS floors that entry. Without
# them the PR deleting this block is exactly the PR that does not schedule this gate.
#
...
def check_self_invocation(
    ...
    # SMA-593 — stripped whole lines too, for both of the release-parity haystack's reasons: the
    # `case` arms and the `if` sit indented inside run.sh, and a commented-out copy must report
    # missing rather than satisfy the pin.
    workflow_credentials_lines = {
        line.strip() for line in workflow_credentials_sh_text.splitlines()
    }
    missing.extend(
        f"ci/workflow-credentials/run.sh: {site}"
        for site in WORKFLOW_CREDENTIALS_SH_CALL_SITES
        if site not in workflow_credentials_lines
    )
    # SMA-603 fix wave — stripped whole lines too, for both of the release-parity haystack's
    # reasons. See RELEASE_PLAN_SH_CALL_SITES' own comment for what each entry closes.
    release_plan_lines = {line.strip() for line in release_plan_sh_text.splitlines()}
    missing.extend(
...
def check_tailwind_guard_invocations(apps, raw_tasks, registry=None):
    """SMA-512, fix wave. Every ts/apps/* app must invoke the tailwind-source guard for its own
    directory, from its Moon project's RESOLVED `test` script.
    ...
    convention (`moon-project-id-stack-suffix`). PURE, so the self-test drives it with plain
    dicts.

    Returns (unregistered, missing_lines, stale, no_project), all sorted:
      - unregistered: an app with no TAILWIND_GUARD_INVOCATIONS entry.
      - missing_lines: "app: line" for each of `_expected_tailwind_lines(app)` absent from the
        REGISTERED app's resolved `test` script (finding A: the expected lines are derived from
        the name, so a script that copied another app's `--app` line, or a registry that dropped
        one of the three lines, both still red here). Matched against `_scripts`-style resolved
        text, never the raw moon.yml file, so a line parked in some OTHER task, or one that never
        runs, cannot satisfy this (finding B).
      - stale: a registry entry naming an app no longer on disk.
      - no_project: an app (registered or not) whose `<dir>-ts` Moon project does not resolve, or
        whose project has no `test` task at all — reported rather than silently skipped, the same
        reasoning `unregistered`/`stale` already apply to the registry (finding B's sub-case).
    """
    registry = TAILWIND_GUARD_INVOCATIONS if registry is None else registry
    apps_set = set(apps)

    unregistered = sorted(app for app in apps_set if app not in registry)
    stale = sorted(set(registry) - apps_set)
    ...
    for app in sorted(apps_set):
        ...
        for want in _expected_tailwind_lines(app):
            if want not in present:
                missing_lines.append(f"{app}: {want}")

    return unregistered, sorted(missing_lines), stale, sorted(no_project)
...
def self_test():
    ...
    # SMA-603 fix wave — the same shape again for ci/release-plan/run.sh, built by joining the
    # registry itself with the indentation the real file carries. Deriving it from the tuple keeps
    # the fixture from drifting out of sync with the pin it exercises; the deletion battery below
    # is what proves each individual entry is checked.
    ...
    # SMA-502 — the same shape again for ci/next-public/run.sh, built by joining the registry
    # itself with the indentation the real file carries. Deriving it from the tuple keeps the
    # fixture from drifting out of sync with the pin it exercises; the deletion battery below is
    # what proves each individual entry is checked.
    ...
    # SMA-593 — the release-parity battery above, repeated for the workflow-credentials haystack.
    # One row per pinned line, so a mutant that widened the match back to "matches anywhere" is
    # caught regardless of which entry it is tested against.
    for _wc_site in WORKFLOW_CREDENTIALS_SH_CALL_SITES:
        _wc_broken = "".join(
            line for line in wired_workflow_credentials.splitlines(keepends=True)
    ...
    # Contamination, both directions: a workflow-credentials site must not be satisfiable from
    # another haystack, and a release-parity site must not be satisfiable from this one. The
    # second direction is what a naive "append the wired text to the broken text" pairing misses
    # if the two newest haystacks were ever read as one.
    if not check_self_invocation(
    ...
    if not check_self_invocation(
        wired, scripts, wired_actionlint,
        "".join(line for line in wired_release_parity.splitlines(keepends=True)
    ...
    # SMA-603 fix wave — the workflow-credentials battery above, repeated for the release-plan
    # haystack. One row per pinned line, so a mutant that widened the match back to "matches
    # anywhere" is caught regardless of which entry it is tested against.
    for _rp_site in RELEASE_PLAN_SH_CALL_SITES:
        _rp_broken = "".join(
            line for line in wired_release_plan.splitlines(keepends=True)
            if line.strip() != _rp_site
        )
        if not check_self_invocation(
    ...
    # Contamination, both directions, exactly as the workflow-credentials pair does it: a
    # release-plan site must not be satisfiable from another haystack, and a workflow-credentials
    # site must not be satisfiable from this one.
    if not check_self_invocation(
        wired + wired_release_plan, scripts, wired_actionlint, wired_release_parity,
        wired_workflow_credentials,
    ...
    if not check_self_invocation(
        wired, scripts, wired_actionlint, wired_release_parity,
        "".join(line for line in wired_workflow_credentials.splitlines(keepends=True)
    ...
    # Whole-LINE, not substring: a commented-out copy of a pinned line must report missing.
    _rp_commented = wired_release_plan.replace(
        "    output)   github_output ;;\n", "    # output)   github_output ;;\n"
    )
    if not check_self_invocation(
        wired, scripts, wired_actionlint, wired_release_parity, wired_workflow_credentials,
    ...
    # The C1 pin, stated as its own row because it is the one entry whose VALUE — not merely its
    # presence — carries the fix. Putting `require_uv` back on the `output)` arm is the SMA-603 C1
    # defect verbatim, and a whole-line comparison is what makes that edit report missing.
    _rp_c1_rearmed = wired_release_plan.replace(
        "    output)   github_output ;;\n", "    output)   require_uv; github_output ;;\n"
    )
    if not check_self_invocation(
        wired, scripts, wired_actionlint, wired_release_parity, wired_workflow_credentials,
    ...
    # SMA-512, fix wave. check_tailwind_guard_invocations: every ts/apps/* app with a
    # package.json must invoke the tailwind-source guard for its own directory, in its Moon
    # project's RESOLVED `test` script (self-test, negative control, and the real `--app <dir>`
    # run naming THAT app) — or the parameterization Task 1 added is unguarded, and a new app
    # could never call the script at all with nothing noticing.
    def tw_tasks(**scripts_by_app):
        """{'<app>-ts': {'test': {'script': text}}} — a raw_tasks fixture, one project per app,
        matching moon's real `{project_id: {task_name: task}}` shape closely enough for this
        PURE function."""
        return {
            f"{app}-ts": {"test": {"script": script}} for app, script in scripts_by_app.items()
        }

    def expect_tailwind(label, apps, raw_tasks_fixture, registry, want):
        got = check_tailwind_guard_invocations(apps, raw_tasks_fixture, registry)
        if got != want:
    ...
    # exact shape of the original defect, where a hand-copied per-app entry could read
    # `--app ts/apps/iam-console` under the `gateway-console` key and MEASURE AS PASSING. The
    # expected lines are now DERIVED from the registry key rather than stored, so this reds: no
    # text typed anywhere in the registry can make `gateway-console`'s expected `--app` line be
    # anything other than `--app ts/apps/gateway-console`.
    _wrong_app_script = "\n".join((
    ...
    expect_tailwind(
        "a script naming another app's --app directory reds",
        ["gateway-console"],
        tw_tasks(**{"gateway-console": _wrong_app_script}),
        {"gateway-console"},
        (
            [],
            [
                "gateway-console: node ../../../ci/tailwind-source/run.mjs "
                "--app ts/apps/gateway-console"
            ],
            [],
            [],
        ),
    )
    # Finding B. The three lines exist verbatim, but parked in a task OTHER than `test` (or one
    # that never runs) — the OLD raw-moon.yml-text match found them anywhere in the file; the
    # resolved `test` script must not.
    expect_tailwind(
    ...
    # Finding B's sub-case, and finding C. An app with a package.json but no resolvable
    # `<dir>-ts` Moon project — no moon.yml at all, or one whose project has no `test` task — is
    # REPORTED under `no_project` rather than silently dropped, whether or not it is registered.
    expect_tailwind(
        "an app with no matching Moon project is reported, not skipped",
        ["mystery-app"], {}, {"mystery-app"}, ([], [], [], ["mystery-app"]),
    )
    expect_tailwind(
        "a Moon project with no test task is reported the same way",
        ["mystery-app"], {"mystery-app-ts": {"build": {"script": "echo hi"}}}, {"mystery-app"},
        ([], [], [], ["mystery-app"]),
    )
    expect_tailwind(
        "no_project fires even when the app is also unregistered",
        ["mystery-app"], {}, set(), (["mystery-app"], [], [], ["mystery-app"]),
    )
