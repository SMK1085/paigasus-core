...
| 7 | Twelve self-tests against fixture tables — extractor, path-filter verdicts, branch-filter verdicts, config allowlist, ci-target floor, invocation allowlist, affected-graph wiring, block execution, kill predicate, affected-smoke block, release guard, cargo-lock step — plus a counter (`SELF_TESTS_RAN`/`SELF_TEST_COUNT`) asserting all twelve ran, and a definition-count check catching a thirteenth table that is defined but never wired into `run_self_tests` (`run.sh --self-test`) |
...
State: CURRENT — twelve fixture tables, twelve mutants (thirteen concurrent `--self-test`
subprocesses in check 9's battery: twelve mutants plus the unmutated control). `--self-test` runs
