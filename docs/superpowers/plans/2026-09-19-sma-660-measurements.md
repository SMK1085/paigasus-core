...
All four agree with the spec's F3 table. Each measured query is reproduced from the
debug-formatted string captured by the site's `PROBE` panic (task 4 report for S1/S2, task 5
report for S3/S4), exactly as printed, with one deliberate change: the panic message
Debug-escapes every literal quote as `\"` (for example `\"membership\"`), and that has been
unescaped to a plain `"` in the table above for readability (for example `"membership"`). The
`\n` sequences, all internal whitespace, and the truncation point (each probe was already cut
off mid-word by the probe's own capture, e.g. ending in `"pro`, `"creat`, `$`, `"system, `) are
untouched — nothing else was normalized or shortened. None of the four sites needed a prefix
change.
...
| V9 | new file + 1 s late racer, locks intact | all five tests pass | Confirmed for all five rewritten tests. With production files clean (locks intact) and a marked 1 s `tokio::time::sleep` as the first statement of each racer, the `tenancy_events_pg` binary (S1, S2) ran 4/4 PASS in 2.257s, and the `authz_system_retirement_pg` binary (S3, S4) ran 18/18 PASS in 4.304s (S3 and S4 each took roughly 2.5s — the 1s sleep plus the poll interval before the racer reaches its blocking statement — well inside the 30s budget). The fifth — task 3's idempotent-put absorb test, `concurrent_put_of_the_same_new_policy_id_is_idempotent_not_a_conflict` — was re-run separately under the same mutation shape: racer B (`put_b`, `authz_policy_store.rs:311`) is a plain `tokio::spawn`, structurally identical to S1-S4's racers, so a marked `tokio::time::sleep(Duration::from_secs(1)).await` was inserted as the first statement of its spawned block and run with `PAIGASUS_REQUIRE_DOCKER=1 cargo nextest run -p paigasus-iam --test authz_policy_store -E 'test(concurrent_put_of_the_same_new_policy_id_is_idempotent_not_a_conflict)' --retries 0`. Result: `PASS [2.024s] (1/1) … concurrent_put_of_the_same_new_policy_id_is_idempotent_not_a_conflict`, `1 test run: 1 passed, 16 skipped` — the wait absorbs a late racer, confirming V9's claim for this test too. The marked line was then deleted and the same command re-run to confirm the test still passes unmutated (`PASS [0.994s]`), and `git status --short` showed the test file clean of the mutation before the fix-wave doc-comment edits were made. |
