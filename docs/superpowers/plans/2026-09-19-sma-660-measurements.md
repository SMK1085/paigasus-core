...
| V9 | new file + 1 s late racer, locks intact | all five tests pass | Confirmed for four of the five rewritten tests (S1-S4); the fifth — task 3's idempotent-put absorb test — was never re-run under a V9-shaped late-racer mutation, so this row does not confirm "all five" as literally as the brief's wording suggests. With production files clean (locks intact) and a marked 1 s `tokio::time::sleep` as the first statement of each racer, the `tenancy_events_pg` binary (S1, S2) ran 4/4 PASS in 2.257s, and the `authz_system_retirement_pg` binary (S3, S4) ran 18/18 PASS in 4.304s (S3 and S4 each took roughly 2.5s — the 1s sleep plus the poll interval before the racer reaches its blocking statement — well inside the 30s budget). `authz_policy_store`'s fifth rewritten test is racer-A-as-raw-insert rather than a `tokio::spawn` racer with the same late-start shape as S1-S4, so it was not put through this same control; its mutation coverage instead comes from V5 (see above), the site-specific analogue of the same "the assertion must actually bite" property. |
