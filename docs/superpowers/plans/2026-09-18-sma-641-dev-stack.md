...
const REQUIRED = [
  ...
  // NOT 'serviceInfo.getServiceInfo' — corrected during execution. The fake's defaults() already
  // answers it from the descriptor setServiceInfo() sets, and dispatch() prefers a scripted
  // handler over defaults(), so scripting it would freeze the gRPC answer while setServiceInfo()
  // kept moving the HTTP one. The shipped test asserts its ABSENCE instead.
