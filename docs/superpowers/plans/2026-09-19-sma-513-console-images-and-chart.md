...
- **An assertion battery must run every row, even after one fails.** Under `set -e`, a bare non-zero command inside a failure branch aborts the whole script, so one red row silently cancels every row after it — measured on `charts/paigasus/tests/refusals.sh`, where a failing render row stopped the run before the last row ever executed. Capture into a variable and test the variable (`if out="$(render "$@")"; then … else printf '%s\n' "$out"; ec=1; fi`); never re-run the failing command bare to print its output. Close with `if [ "$ec" -eq 0 ]; then echo "== … OK =="; fi` rather than `[ "$ec" -eq 0 ] && echo …`, whose failing test is itself a non-zero top-level list under `set -e`. This applies to `maps.sh`, `ingress.sh` and `render.sh` in Tasks 9, 12 and 13 as well.
- **A script that takes `--set` flags must forward them.** Bash functions get their own positional parameters, so a helper called with row-specific flags never sees the script's own `$@`. Capture `BASE=("$@")` at the top and splice it into every render: `helm template … "${BASE[@]}" "$@"`. Without this, a script invoked exactly as its own definition-of-done prescribes fails every row that needs a required value.
