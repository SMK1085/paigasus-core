...
- `plugins: ["<in-repo-path-filter>", "@semantic-release/commit-analyzer"]` — the in-repo path-filter restricts the analyzed commits to those touching the package directory, then `@semantic-release/commit-analyzer` classifies. **No `releaseRules`** and the default conventional preset → native strict-semver classification (the documented divergence). No `@semantic-release/npm` / `@semantic-release/github` (those are publish/push plugins; activation and their tokens are SMA-407's job).
...
- **`ecosystem::run_update dir`** — for **each** slot, `cd` into the slot's package dir and run `semantic-release --dry-run --no-ci` (resolved binary, §5). The in-repo path-filter restricts the analyzed commits to that package dir, so slot `a` (a commit under `a/`) bumps and slot `b` (no commit under `b/` since its baseline tag) reports "no relevant changes" → baseline. Parse the next version from the dry-run output (Risk 3) and write it to a per-slot sentinel `$dir/$slot/.parity-next-version`. **Both** slots run through semantic-release, so `b` is a genuine assertion, not a hardcoded constant. Capture stderr and replay on real failure (mirrors both existing adapters).
...
### 5. Binary + plugin resolution

semantic-release and its plugins are **Node** tooling → installed as `ts/` pnpm dev-dependencies (`ts/package.json`), pinned in `ts/pnpm-lock.yaml`. CI already runs `pnpm --dir ts install --frozen-lockfile`, so they are present before `moon ci`.

The fixture lives in `/tmp`, outside `ts/`, where neither the `semantic-release` binary nor its ESM plugin imports resolve by default (the same out-of-repo trap both existing adapters document). Resolution (Risk 2):

- **Binary** — resolve the absolute `semantic-release` from `ts/` once (e.g. `node "$ts/node_modules/.bin/semantic-release"` or `pnpm --dir ts exec which semantic-release`), mirroring how `release-plz.sh` resolves `RELEASE_PLZ_BIN` and PSR resolves `PSR_BIN`.
- **In-repo path-filter** — referenced by **absolute path** in the fixture `.releaserc.json` (it lives under `ts/`, e.g. `ts/tooling/semantic-release-path-filter.mjs`), so it resolves regardless of the fixture's `/tmp` cwd with no `node_modules` dependency at all.
- **`@semantic-release/commit-analyzer`** — semantic-release (ESM) resolves plugins relative to cwd, which in the fixture is `/tmp/.../a`. **Primary:** reference it by absolute path resolved via `node -e "process.stdout.write(require.resolve('@semantic-release/commit-analyzer'))"` run from `ts/` — this bypasses semantic-release's cwd-relative ESM resolution entirely and is agnostic to pnpm's symlinked `.pnpm` layout. **Fallback:** symlink `ts/node_modules` into the fixture root (`$dir/node_modules`) and let resolution walk up (more exposed to pnpm's symlink-farm layout from a `/tmp` cwd, hence the fallback, not the primary).
...
2. **Plugin resolution from `/tmp`.** semantic-release (ESM) resolves plugins relative to cwd; the fixture is outside `ts/`. The in-repo path-filter is referenced by absolute path (no resolution concern). For `@semantic-release/commit-analyzer`: **primary** is an absolute path via `require.resolve` from `ts/` — it bypasses semantic-release's cwd-relative ESM resolution entirely and is agnostic to pnpm's symlinked `.pnpm` layout. **Fallback:** symlink `ts/node_modules` into the fixture root and let resolution walk up (more exposed to pnpm's symlink-farm layout from a `/tmp` cwd — hence demoted from the primary it held in an earlier draft) (§5).
3. **Dry-run output parsing.** Confirm `semantic-release --dry-run` prints a stable `the next release version is X.Y.Z` line (case-insensitive match) and a recognizable "no relevant changes" line for the no-bump slot. Trim and assert `^[0-9]+\.[0-9]+\.[0-9]+$`. Fallback for a no-bump slot: treat a slot with no commit touching its dir since its baseline tag (`git log <tag>..HEAD -- <dir>` empty) as baseline — the git-log guard demoted to a no-bump-output fallback, exactly as PSR §2 does.
4. **Offline / no-token / no-CI.** With a minimal `plugins` set (no `@semantic-release/npm` / `@semantic-release/github`), dry-run should perform no registry/token verification. Confirm `--dry-run --no-ci` needs no network and that semantic-release does not error on the placeholder `origin` remote.
