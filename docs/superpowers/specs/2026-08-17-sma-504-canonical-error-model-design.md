...
- `ERROR_REASON_MISSING_AUTH_CONTEXT = 33` (IAM range; 32 is the current maximum). An internal invariant violation, exactly like `missing-scope`: it means `AuthLayer` did not attach a context. `retryable = unknown`.
- `ERROR_REASON_CAPABILITY_DISABLED = 903` (**shared** range; 902 is the current maximum), with the capability name in `ErrorInfo.metadata["capability"]`. Shared rather than IAM-only because capability gating is not IAM-specific, and metadata-carried rather than one reason per capability because the registry is append-only and capabilities are not.
