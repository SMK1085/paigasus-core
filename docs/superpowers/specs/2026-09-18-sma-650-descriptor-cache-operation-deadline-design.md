...
| D7 | The abandoned `inner` promise gets a `.catch(() => undefined)`. A late success is discarded and never cached. | § 2 fact 7: node-redis rejects it when the socket dies. `Promise.race` already attaches a handler to `inner`, so this `.catch` closes no live unhandled-rejection path. It is belt-and-braces: it documents that the abandoned operation's late outcome is deliberately ignored, and it keeps that true if the race is ever refactored away. A late success is also discarded and never cached: the caller has already degraded and moved on. |
...
| U9 | The abandoned promise's late rejection surfaces nowhere. It does NOT prove an unhandled rejection is prevented, because `Promise.race` already handles the rejection and none can occur. |
