...
> **ADR-0011 wording note:** ADR-0011 S1/S2 originally framed *both* kernel and proto Py packages as "maturin byproducts." That is precise for the kernel but not for proto (codegen, not maturin). **This has been corrected in ADR-0011 (amended 2026-06-03):** the proto Py/TS packages are now described as **buf-codegen** byproducts of `contracts/`. The conclusion is unaffected for SMA-405; the distinction matters for **E-activate**, whose proto version propagation rides the codegen pipeline, not a maturin build.
...
### 3. Fixture config derived from the real configs, with a cross-package equality guard (F3)
