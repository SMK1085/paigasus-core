...
| PyPI package uninstallable off linux/x86_64 | **Closed by SMA-578** (was: blocked out to SMA-407c, not claimed as closed) — a six-leg / seven-wheel matrix (darwin arm64 + x86_64, win x86_64, manylinux + musllinux on x86_64 and aarch64) **plus** an sdist verified from source on macOS, Windows and Linux. The "no sdist" half of §7's original mitigation rested on a premise measured false; see §7's correction block. |
