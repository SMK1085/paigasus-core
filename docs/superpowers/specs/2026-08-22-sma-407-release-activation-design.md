...
**Verified against the pinned release-plz `0.3.158`** (`.prototools`), not assumed:
`version_group` is accepted (a probe config parsed and execution proceeded past config loading),
and release-plz uses `deny_unknown_fields` — a deliberately misspelled `version_grouppp` fails
hard at TOML parse — so acceptance is a real pass, not a silent ignore.
...
| 7 | `rs/Cargo.toml` `[workspace.dependencies] paigasus-kernel.version` | kernel | **release-plz (verify) / `--write`** |
| 8 | `rs/Cargo.toml` `[workspace.dependencies] paigasus-proto.version` | proto | **release-plz (verify) / `--write`** |
| 9 | `rs/Cargo.toml` `[workspace.dependencies] paigasus-proto-derive.version` | proto | **release-plz (verify) / `--write`** |
...
resolves against a crates.io version that will never exist. Whether release-plz rewrites them
automatically is the single measurement the plan must take first (§12).
...
only for the seven family members, `release = false` (or at minimum `git_tag_enable = false`) for
everything else. The spec must also state what version the service crates sit at afterwards and what
that means for ADR-0020.
...
1. Does release-plz rewrite `[workspace.dependencies]` version requirements (sites 7–9)? **Measure
   first** — the fallback is widening `--write`.
2. Does `version_group` hold crates whose *Cargo manifest* says `publish = false` (the three
   binding crates), or only crates the *release-plz config* marks unpublished?
3. What is `git_tag_name`? `rs/release-plz.toml` sets none; the default is `{package}-v{version}`.
   Does it collide with napi's `lerna` style (`@paigasus/node-bindings@0.1.0`)?
4. What schedules the `release` job, and is `release_always` on or off?
5. How does the *first* `cargo publish --dry-run -p paigasus-proto` behave before
...
6. Does the generated per-crate `CHANGELOG.md` ship? `paigasus-kernel`'s `include` allowlist
   (`Cargo.toml:19`) excludes it, and new tracked files have `repo:input-liveness` and Check 2b
   consequences.
7. `paigasus-py-bindings/pyproject.toml` has essentially no PyPI metadata and no LICENSE/README.
