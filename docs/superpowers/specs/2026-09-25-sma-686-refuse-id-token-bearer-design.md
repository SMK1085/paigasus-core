...
| D2 | IAM refuses a token whose payload `typ` claim is a string equal to `ID` or `Logout`, ASCII case-insensitive. This is the only marker. (Amended by D12: the standard logout markers.) | Sven chose "Keycloak `typ` only" on 2026-09-25, after the Dex measurement. No measured access token has either value. The rule is specific to Keycloak and to IdPs that copy its `typ` claim. The case-insensitive compare protects against a variant spelling. |
...
| D4 | The header `typ` is **not** read. (Amended by D12: the header `typ` is read for `logout+jwt` only.) | Keycloak sends `JWT` on both tokens by default (§ 2). An `at+jwt` header is not required (D1). |
...
> This section describes the first implementation. D12-D15 (§ 10) changed the code:
> `check_access_token_type` became `non_access_token_marker` plus `log_refusal`.
